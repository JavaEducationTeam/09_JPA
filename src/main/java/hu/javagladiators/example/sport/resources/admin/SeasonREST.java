 ...
 */
@Path("/season")
@Api(value = "Season administration")
public class SeasonREST {
    ...
    public List<IdNamePOJO> idnemeList(){
        ...
        List<Season> entities = service.getAll();
    ...
    @Path("/{id}") 
    @Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
    public Season findById(@PathParam("id") int id){
   ...
   @POST
    ...
    public MessagePOJO save(NameDescriptionPOJO pData){
        service.save(dtoBasicIdNameDescription.toSeason(pData));
